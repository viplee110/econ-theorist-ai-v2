"""Export strict Phase 2 theory-payload schemas independently of core v1."""
...
from econ_theorist.theory import THEORY_PAYLOAD_MODELS  # noqa: E402
...
def rendered_schemas() -> dict[str, str]:
    ...
    for entity_type, model in sorted(THEORY_PAYLOAD_MODELS.items()):
        filename = schema_filename(entity_type)
        schema = model.model_json_schema(mode="validation")
        schema["$id"] = f"https://econ-theorist.ai/schemas/theory/v1/{filename}"
...
def check(destination: Path) -> bool:
    ...
    if actual_names != set(expected):
        return False
    return all(
        (destination / filename).read_text(encoding="utf-8") == text
        for filename, text in expected.items()
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    destination = ROOT / "schemas" / "theory" / "v1"
    if args == ["--check"]:
        return 0 if check(destination) else 1
    if args:
        raise SystemExit("usage: export_theory_schemas.py [--check]")
    export(destination)
    return 0
