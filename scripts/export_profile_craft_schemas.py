"""Export strict Phase 4 profile/craft schemas independently."""
...
from econ_theorist.profile_craft import PROFILE_CRAFT_PAYLOAD_MODELS  # noqa: E402


SCHEMA_ID_ROOT = "https://econ-theorist.ai/schemas/profile-craft/v1"
...
def rendered_schemas() -> dict[str, str]:
    ...
    for entity_type, model in sorted(PROFILE_CRAFT_PAYLOAD_MODELS.items()):
...
def main(argv: list[str] | None = None) -> int:
    ...
    destination = ROOT / "schemas" / "profile_craft" / "v1"
    if args == ["--check"]:
        return 0 if check(destination) else 1
    if args:
        raise SystemExit("usage: export_profile_craft_schemas.py [--check]")
    export(destination)
    return 0
