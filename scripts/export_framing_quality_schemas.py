"""Export strict v5 framing-quality schemas independently."""
...
SCHEMA_ID_ROOT = "https://econ-theorist.ai/schemas/framing-quality/v1"
...
def rendered_schemas() -> dict[str, str]:
    ...
    for entity_type, model in sorted(FRAMING_QUALITY_PAYLOAD_MODELS.items()):
...
def main(argv: list[str] | None = None) -> int:
    ...
    destination = ROOT / "schemas" / "framing_quality" / "v1"
    if args == ["--check"]:
        return 0 if check(destination) else 1
    if args:
        raise SystemExit("usage: export_framing_quality_schemas.py [--check]")
    export(destination)
    return 0
