...
class Phase5A2CodexSkillTests(unittest.TestCase):
    def test_projection_is_thin_and_engine_owned(self) -> None:
        ...
        self.assertIn("For a v2 overlay", text)
        self.assertIn("`Champion`, `Serious runner-up`", text)
        self.assertIn("`Eliminated candidates`", text)
        self.assertIn("never request private reasoning", text)
        self.assertIn("invent scores, or choose for ease", text)
        self.assertIn("collaborator `Champion` sections", text)
        self.assertIn("covers only that lane's v2 `Champion`", text)
        self.assertIn("A runner-up needs a new bound source review", text)
