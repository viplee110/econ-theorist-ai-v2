...
def validate_route_registry(registry: RouteRegistryLike) -> RouteRegistryLike:
    ...
    if registry.registry_version == 5:
        expected_versions = {
            route_id: (
                5
                if route_id in V5_NATIVE_ROUTE_IDS
                else 4
        ...
        if wrong_versions:
            raise RegistryError(
                "registry v5 may advance only its framing-audit and framing-repair "
                "routes to version 5"
            )

        frozen_v4 = load_route_registry(_routes_root() / "registry.v4.json")
        frozen_by_id = {route.route_id: route for route in frozen_v4.routes}
        for route in registry.routes:
            if route.route_id in V5_NATIVE_ROUTE_IDS:
                continue
            if route.model_dump(mode="json") != frozen_by_id[route.route_id].model_dump(
                mode="json"
            ):
    ...
    if registry.registry_version == 7:
        expected_versions = {
            route_id: (
                7
                if route_id in V7_NATIVE_ROUTE_IDS
                else 5
                if route_id == "repair.dependency"
                else 4
                if route_id in V4_NATIVE_ROUTE_IDS
        ...
        if wrong_versions:
            raise RegistryError(
                "registry v7 may advance only the framing-economics audit "
                "to route version 7"
            )

        frozen_v6 = load_route_registry(_routes_root() / "registry.v6.json")
        frozen_by_id = {route.route_id: route for route in frozen_v6.routes}
        for route in registry.routes:
            current = route.model_dump(mode="json")
            frozen = frozen_by_id[route.route_id].model_dump(mode="json")
            if route.route_id == "audit.framing_economics":
                for field in (
                    ...
                    current.pop(field)
                    frozen.pop(field)
                if route.instruction_bundle_id != "audit.framing_economics.v7":
                    raise RegistryError(
                        "registry v7 framing audit must bind its v7 instruction"
                    )
            if current != frozen:
                raise RegistryError(
